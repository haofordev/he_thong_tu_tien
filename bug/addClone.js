...
const GIFT_CODES = [
    ...
    'TATUTIEN', 'TATRUNGSINH', 'SAMSARA2026'
];
...
async function sellAllItems(token, charId) {
    try {
        const invRes = await fetch(`${BASE_URL}/rest/v1/rpc/rpc_list_inventory`, {
            method: 'POST',
            headers: { ...HEADERS_BASE, authorization: `Bearer ${token}`, 'content-profile': 'public' },
        ...
        log(`Đang treo bán ${sellableItems.length} loại vật phẩm giá 1 đá...`);
        for (const item of sellableItems) {
            try {
                ...
                const data = await res.json();
                if (data?.ok) {
                    log(`Đã treo bán: ${item.name} | Listing ID: ${data.listing_id}`, 'success');
                } else {
                    log(`Treo bán lỗi [${item.name}]: ${JSON.stringify(data)}`, 'warning');
                }
            } catch (e) {
                log(`Treo bán Crash: ${item.name} - ${e.message}`, 'error');
            }
            await sleep(1000);
        }
    } catch (e) {
        log(`Lỗi tổng khi treo bán: ${e.message}`, 'error');
    }
}
...
async function processOne(names) {
    ...
    await sleep(1000);
    await sellAllItems(token, charId);
